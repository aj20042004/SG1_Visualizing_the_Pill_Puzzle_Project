...
def compute_half_draw_probability(N, D, R):
    # Counter to track how many simulations drew a half pill on day D
    half_draw_count = 0

    # Run the simulation R times
    for i in range(R):
        _, _, _, HalfDrawOnDayD = simulate(N, D)

        # If a half pill was drawn on day D in this simulation, increment counter
        if HalfDrawOnDayD:
            half_draw_count += 1
...
if __name__ == "__main__":
    # Example values for testing Q3
    N = 2
    D = 3
    R = 10

    compute_half_draw_probability(N, D, R)